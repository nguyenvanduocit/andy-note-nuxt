...
// highlight / popover lives here; the data/identity concern is in useComments
// (Firebase Auth + Firestore via VueFire).
//
// Mounted CLIENT-ONLY and only when the feature is enabled (its parent
// CommentLayer.vue gates both), once per column inside ContentView's article
// view, and scoped to THAT column's rendered `.content` element — so the same
// article shown in two stacked columns highlights independently in each.
//
// Code-review-style flow:
//   • select text in the article → floating "Comment" button → composer popover
//   • posting requires Google sign-in; the author name comes from the token
//   • a stored comment paints a coral <mark>; clicking it opens its thread
//   • the site owner clicks Resolve → it disappears for everyone (live)
...
const { user, isOwner, comments, signIn, signOutUser, postComment, resolveComment }
  = useComments(props.path)
...
function timeAgo(ts: Timestamp | null): string {
  const ms = ts ? ts.toMillis() : Date.now()
...
// Firestore is live: the roll updates in real time as comments are posted or
// resolved (here or in another tab), and every change re-paints the highlights.
watch(comments, () => nextTick(repaint))
...
async function submitDraft(): Promise<void> {
  ...
  try {
    await postComment(body, anchor)
    // The live query adds the new doc and the watcher repaints — no local push.
...
async function resolve(id: string): Promise<void> {
  try {
    ...
    // The live query drops the doc and the watcher repaints — no local filter.
...
onMounted(async () => {
  ...
  // Paint whatever the live query already delivered; later changes go through
  // the `comments` watcher.
  repaint()
})
    ...
    <section class="ec-panel" aria-label="Reader comments">
      ...
      <ul v-else class="ec-roll">
        <li v-for="c in comments" :key="c.id" class="ec-roll__item">
          ...
          <button v-if="isOwner" type="button" class="ec-resolve" @click="resolve(c.id)">
            ✓ Resolve
          </button>
        </li>
      </ul>
        ...
        <template v-if="popMode === 'compose'">
          ...
          <template v-if="user">
            ...
            <p class="ec-pop__who">
              ...
              <button type="button" class="ec-pop__signout" @click="signOutUser">Sign out</button>
            </p>
          ...
          <template v-else>
            <p class="ec-pop__signin-msg">Sign in with Google to comment.</p>
            <div class="ec-pop__actions">
              <button type="button" class="ec-btn" @click="closePopover">Cancel</button>
              <button
                type="button"
...
/* "Commenting as …" line under the textarea. */
.ec-pop__who {
  display: flex;
  align-items: center;
  gap: 0.4rem;
