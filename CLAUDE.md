...
- **Title** — `ContentView` always feeds seo-utils a raw `displayTitle` (body H1 → frontmatter `title` → slug), never the bare `page.title`. seo-utils applies its default `%s | %siteName` template, appending the site name exactly once. When `displayTitle` equals the site name (typical for the root index, whose H1 is the brand), `ContentView` emits an empty title instead — the template trims the dangling separator and renders the site name once, never a doubled `Site | Site`. The comparison uses `useSiteConfig().name` (what `%siteName` resolves from), not `runtimeConfig.public.site.title`. Do **not** reintroduce a static `app.head.title`: on a listing route with no page title it flows back through that template and renders a doubled `Site — X · Site`.
...
**The root index is permanent column 0.** `useStack.fullStack` always prepends `/` (and the current `route.path` when non-root), so a deep-linked article never renders as a lone, navigation-less column. Column 0 is implicit — never serialized into `?stack=` — keeping canonical article URLs clean. `pushColumn` encodes the rest back into the URL: `route.path` stays the URL path while it survives the trim; clicking a link inside the root column while deep-linked navigates to the target's own path. SEO depends on this: every column registers `useSeoMeta` in the *synchronous* part of setup (before its data await) so unhead's entry-order dedupe deterministically resolves to the deepest column — move those calls below an `await` and prerendered articles can ship the homepage's title.

**Two layouts, one state machine.** The `?stack=` query and the push/trim logic are viewport-agnostic; only the rendering + scroll axis branch at `(max-width: 767px)` (= Tailwind `md`). Desktop (`md`+) is the horizontal **sticky-peek** stack: columns are fixed-width, full-height, peek `--stack-peek` at the LEFT (`position: sticky; left: idx*peek`), and each scrolls its article in an internal body — the page x-scroll and the column y-scroll are orthogonal, so they never fight. Mobile is a **single vertical scroll**: that nested arrangement rotated would put the column's inner scroll on the *same* axis as the page scroll (two competing scrolls — janky on touch), so instead columns flatten into full-width sections flowing at natural height in one y-scroller, each header `sticky; top: 0`. The mobile branch lives in three coordinated places: `StackedColumns.vue` makes the container a y-scroller (`flex-col`/`overflow-y-auto`, `md:` restores x); `StackedColumn.vue`'s `@media` block makes each column `display:block; height:auto; position:static`; and `main.css` `.column-pane` / `.column-pane__scroll` (carried by `ContentView.vue` + `TagListing.vue`) release the inner scroll so the article flows + pin the header. `scrollToColumn` in `useStack.ts` branches to scroll the column's top to the container top. Keep the `767px` CSS breakpoint and the `matchMedia` query in lockstep.
